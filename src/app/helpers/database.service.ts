...
import { CollectionReference, DocumentData, Query, WriteBatch } from '@firebase/firestore-types';
...
export class DatabaseService {
    ...
    public getCollection<T>(collection: string, filtros?: QueryFn) {
        return this.afs
            ...
            .snapshotChanges()
            .pipe(map((value) => value.map((snap) => ({ ...snap.payload.doc.data(), id: snap.payload.doc.id } as T))));
    }
    ...
    public async save<T>(collection: string, newValue: any, oldValue?: any): Promise<T> {
        newValue.key;
        const docRef: DocumentReference = await this.getDocument(collection, newValue.key || newValue.id || null);
    ...
    public async update<T>(collection: string, id: string, value: any): Promise<T> {
        return { ...value, id };
    }
    ...
    async list<T>(
        collection: string,
        orderBy?: string | Array<string> | { key: string; direction: 'asc' | 'desc' },
    ...
    private queries(
        values: firebase.firestore.CollectionReference<firebase.firestore.DocumentData> | Query<DocumentData>,
        where: any[] | undefined,
        orderBy: string | any[] | { key: string; direction: firebase.firestore.OrderByDirection } | undefined,
    ...
    private async setItem<T>(docRef: firebase.firestore.DocumentReference, saveValue: any, collection: string) {
        if (!navigator.onLine) {
            // Quando está offline
            this.promises.push(docRef.set(saveValue) as never);
            if (this.debug) {
                console.log('Document ' + collection + '/' + docRef.id + ' written successfully in offline');
            }
            ...
            return docRef
                ...
                .then(() => {
                    if (this.debug) {
                        console.log('Document ' + collection + '/' + docRef.id + ' written successfully');
                    }
                    return { ...saveValue, id: docRef.id };
                })
                .catch((error) => {
