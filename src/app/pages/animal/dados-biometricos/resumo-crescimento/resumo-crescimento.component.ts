...
export class ResumoCrescimentoComponent implements OnInit {
  ...
  alturas: any[] = [];
  alturasData: any[] = [];
  constructor(private service: DadosBiometricosService, private animalService: AnimalService) { }

  ngOnInit(): void {
    this.service.listar([{key: 'animal.nome', op:'==', value: this.animalService.animal.nome}], '').then(doc => {
      this.pesos = []
      this.pesosData = []
