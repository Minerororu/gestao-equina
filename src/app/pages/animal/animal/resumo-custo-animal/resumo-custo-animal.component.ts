...
export class ResumoCustoAnimalComponent implements OnInit {
  ...
  constructor(private service : AnimalService, private lancamentoService: LancamentoFinanceiroService) { }

  ngOnInit(): void {
    if(this.service.animal){
      this.animal = this.service.animal;
      this.lancamentoService.listar([{key: 'animal.id', op:'==', value: this.animal?.id}]).then(doc => {
        this.lancamentos = doc as Lancamento[];
        this.lancamentos.map(lancamento => {
          this.labels.includes(lancamento.centroDeCusto?.nome)?'' : this.labels.push(lancamento.centroDeCusto?.nome);
          if (this.dataObj.filter(e => e?.centroCusto === lancamento?.centroDeCusto?.nome).length == 0) {
            this.dataObj.push({centroCusto: lancamento.centroDeCusto?.nome, valor: lancamento?.valorDocumento});
          }else{
            var index = this.dataObj.map(function(e) { return e.centroCusto; }).indexOf(lancamento.centroDeCusto?.nome);
