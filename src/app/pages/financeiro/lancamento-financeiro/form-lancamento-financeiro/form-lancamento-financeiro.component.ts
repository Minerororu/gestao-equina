...
export class FormLancamentoFinanceiroComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.lancamento = this.service.lancamento
      ? this.service.lancamento
      : new Lancamento();
  ...
  debitar_creditar(lancamento) {
    ...
    if (lancamento.operacao == 'Crédito') {
      ...
      if (conta.saldo - lancamento.valorDocumento >= 0) {
        ...
        alert(
          `A conta: ${conta.nome}, não tem saldo suficiente para realizar a operação`
        );
  ...
  submit(lancamento) {
    if (this.canSubmit) {
      lancamento.pagamentoEfetuado ||
      (lancamento.pagamentosEfetuados >= 1 &&
        this.oldLancamento.valorDocumento != lancamento.valorDocumento &&
        !this.editing)
        ? this.debitar_creditar(lancamento)
        : '';
