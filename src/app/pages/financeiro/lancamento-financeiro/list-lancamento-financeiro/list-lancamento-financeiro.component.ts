...
export class ListLancamentoFinanceiroComponent implements OnInit {
  ...
  dataInicioDate: any;
  dataFinalDate: any;
  constructor(private service: LancamentoFinanceiroService, private router: Router, private form: FormLancamentoFinanceiroComponent) { }
  ...
  ngOnInit(): void {
    this.service.listar().then(doc => {
      ...
      this.lancamentos.map(lancamento => {
        lancamento.quantidadeRepeticao > lancamento.pagamentosEfetuados? this.repetirPagamento(lancamento):'';
      })
    })
  }
  ...
  repetirPagamento(lancamento: Lancamento) {
    ...
    switch(lancamento.repetir){
      case 'Semanalmente': dataLimite.setDate(dataInicio.getDate() + 7);break;
      case 'Quinzenalmente': dataLimite.setDate(dataInicio.getDate() + 15);break;
      case 'Mestralmente': dataLimite.setMonth(dataInicio.getMonth() + 1);break;
      case 'Bimestralmente': dataLimite.setMonth(dataInicio.getMonth() + 2);break;
      case 'Trimestralmente': dataLimite.setMonth(dataInicio.getMonth() + 3);break;
      case 'Semestralmente': dataLimite.setMonth(dataInicio.getMonth() + 6);break;
      case 'Anualmente': dataLimite.setFullYear(dataInicio.getFullYear() + 1);break;
    }
    ...
    if(JSON.parse(JSON.stringify(strToDate(hoje()))) >= JSON.parse(JSON.stringify(dataLimite)) && !lancamento.filho){
