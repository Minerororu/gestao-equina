    ...
    this.lancamentoService.listar().then((doc) => {
      ...
      this.lancamentosFiltrados.map((lancamento) => {
        ...
        if (this.demonstrativo.componentes == 'Grupos de Centros') {
          !this.labels.includes(lancamento.centroDeCusto?.grupo.nome) &&
          lancamento.centroDeCusto?.grupo.nome
            ? this.labels.push(lancamento.centroDeCusto?.grupo.nome)
            : '';
          if (
            this.dataObjArr.filter(
              (e) => e?.grupo === lancamento?.centroDeCusto?.grupo.nome
            ).length == 0
          ) {
            this.dataObjArr.push({
              grupo: lancamento.centroDeCusto?.grupo.nome,
              valor: lancamento?.valorDocumento,
            });
          } else {
            var index = this.dataObjArr
              ...
              .indexOf(lancamento.centroDeCusto?.grupo.nome);
            this.dataObjArr[index].valor += lancamento?.valorDocumento;
          }
        }
      });
    ...
    if (this.demonstrativo.componentes == 'Centros Financeiros') {
      this.demonstrativo?.centrosDeCustos.map((centro) => {
        this.lancamentos.map((lancamento) => {
          lancamento.centroDeCusto?.nome == centro.nome
            ? this.lancamentosFiltrados.push(lancamento)
            : '';
        });
      });
    } else {
      this.demonstrativo?.grupos.map((grupo) => {
        this.lancamentos.map((lancamento) => {
          lancamento.centroDeCusto?.grupo.nome == grupo.nome
            ? this.lancamentosFiltrados.push(lancamento)
            : '';
        });
      });
