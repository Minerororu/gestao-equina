    ...
    this.service.listar().then(doc => {
      ...
      this.vacinacoes.map(vacinacao  => {
        if(vacinacao.vacina.periodo){
          ...
          dataVacinacao.setMonth(dataVacinacao.getMonth() + vacinacao?.vacina?.periodo)
          //comparando a data
          if(dataVacinacao <= hojeDate){
            this.animalTemp = vacinacao.animal.nome;
            this.alerta = true;        
            this.vacinaTemp = vacinacao.vacina.vacina;
          }else if(this.animalTemp == vacinacao.animal.nome && this.vacinaTemp == vacinacao.vacina.vacina){
            this.alerta = false;
          }
        }
      })
      this.alerta ? alert(`O animal ${this.animalTemp}, precisa tomar a vacina: '${this.vacinaTemp}'`) : ''
    })
  }
