...
export class FormControleFolicularComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.controle = this.service.controle ? this.service.controle: new ControleFolicular();
    this.controle.dadosControle? '' : this.controle.dadosControle = {responsavel: null, data: null, procedimento: null, descricao: null, ovarioDireito: null, ovarioEsquerdo: null, observacoes: null, utero: null} 
  }
  ...
  submit(){
    if(this.service.bloquearEdicao && this.controle.bloquearEdicao){
      ...
      this.controle?.dadosControle.procedimento ? this.criarUsoProcedimento():'';
      this.service.salvar(this.controle);
      this.router.navigate(['list-controle-folicular']);
    }
  }
  criarUsoProcedimento() {
    alert(`O animal ${this.controle.animal.nome}, acaba de usar o procedimento: ${this.controle.dadosControle.procedimento.nome}!`);
